...
export function resolveModelId(config?: NvidiaClientConfig): string {
  return config?.model ?? process.env['NVIDIA_NEMOTRON_MODEL'] ?? DEFAULT_NEMOTRON_MODEL;
}
...
export async function callNvidiaApi(
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      return {
        success: false,
        error: err instanceof Error ? err.message : 'Unknown error calling NVIDIA API',
      };
    }
  }

  return lastError ?? { success: false, error: 'Unknown error calling NVIDIA API' };
}
