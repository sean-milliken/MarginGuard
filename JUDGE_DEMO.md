...
From the repository root, run `npm ci`, `npm run build:demo`, then `npm start`. Open http://127.0.0.1:3001 and choose **Open Dashboard**. For development with automatic reload, use `npm run dev` and http://127.0.0.1:5173 instead. Stop the active server with Ctrl+C before starting another on the same port.

## Two-minute walkthrough

1. Choose **Run Judge Demo**. The application loads the supplied synthetic freight brief, resolves dependencies, runs the existing financial engine, and persists the analysis through the existing API. The result opens Event analysis immediately after successful persistence. This offline fixture does not claim a live Nemotron inference or model confidence.
...
6. Open **Scenarios** and choose **Leadership announcement · no impact**. The same scenario endpoint, finance engine and persistence path return zero affected units, zero exposure, and no action required. To test actual model classification of this source, open Intelligence, use the scenario example, and analyze it with a configured NVIDIA key.
7. Open **Evals**. It leads with measured classification accuracy, relevance F1, and schema validity when results are available. Dataset composition and failures expand on request. It automatically loads the genuine local `nemotron/eval-results.json` produced by the harness, or accepts an uploaded results file, to see measured metrics and inspect incorrect predictions. The results file is ignored by Git so that each run remains attributable to the configured model and key.

## Real Nemotron evaluation

Set `NVIDIA_API_KEY` in the root `.env` (never commit it) or export it in your shell. Run `npm run eval:nemotron`. The existing harness calls NVIDIA for each labeled example and writes `nemotron/eval-results.json`, which is ignored by Git. The local Evals page loads that file automatically; an upload control supports other recorded runs. The viewer validates the format and dataset IDs and recomputes metrics from predictions rather than trusting saved percentage claims. The harness now retains expected labels and source excerpts alongside each prediction/failure.
...
- What-if values are local, unsaved assumptions. Custom saved inputs are on Scenarios; calculations, evidence, and processing details expand on request. A new scenario or Judge Demo resets local what-if state.
